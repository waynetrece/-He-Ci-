...
## 最新狀態：報價單草案 v8 已提交，正在整理 v9 前的本機修訂
...
- Front matter：`status: 草案v8`、`draftedAt: 2026-05-05`
- 依據：
  - `HJ網站客製需求表_1150422`
  - `32題客戶review_2026-05-04`
- v8 最新提交重點：
  - 拿掉 FAQ，因 FAQ 不在 HJ 原始需求表 `1150422` 內。
  - 形象 CMS / 單層頁面編輯依需求收斂。
  - 全站搜尋來源去除 FAQ。
  - v8 已記錄主案未稅 `$1,070,750`、含稅 `NT$ 1,124,288`、主案 + 選配 A+B+C 含稅 `NT$ 1,255,413`。

### 目前本機未提交修訂重點

`discussions/hj-quote-draft.md` 目前有本機修改，方向看起來是：

- 設計區：形象 / CMS 內頁由 6 頁改 4 頁。
- 設計小計由 `$170,500` 改為 `$161,500`。
- 最新消息管理併入「行銷活動 / 部落格管理」，首頁改成抓取最新部落格 / 活動公告。
- 行銷活動文案改為「部落格 / 活動公告」列表與詳情。

但這份本機修訂尚未可交付 / 尚未可提交：

- 多個程式區項目編號被改成重複的 `13`。
- 報價總計區仍顯示設計區 `$170,500`，與前面新設計小計 `$161,500` 不一致。
- 若接受設計小計 `$161,500`，主案總計、稅額、含稅總計、付款分期、選配合計都需要重新核算。

### 下一步

1. 先決定是否保留目前本機修訂方向：CMS 4 頁、最新消息併入行銷活動 / 部落格。
2. 若保留，先清理 `discussions/hj-quote-draft.md`：
   - 修正項目流水號。
   - 重算設計區 / 主案未稅 / 營業稅 / 含稅總計 / 50-30-20 付款金額 / 主案 + 選配合計。
   - 將 front matter 狀態從 `草案v8` 改成下一版，例如 `草案v9`。
3. 清理後再決定是否輸出客戶版 Word / PDF。
4. mockup 仍先不要主動改；目前主線是報價單與需求範圍收斂。

### 不要做的事

- 不要把目前本機未提交的 `hj-quote-draft.md` 當作可直接交付版。
- 不要在未確認金額前產出對外報價檔。
