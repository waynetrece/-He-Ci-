...
目前主線是 HJ 報價單草案整理；`discussions/hj-quote-draft.md` 已更新為草案 v12 工作稿，參考馬亞既有報價單語氣，但依 HJ 架構重新撰寫設計與程式內容。v12 主案未稅 `$1,049,750`、含稅 `NT$ 1,102,238`，待 Wayne review 後才輸出正式客戶版。
...
- v12 已修正：項目流水號、設計/程式小計、稅額、50/30/20 付款金額、樣品需登入、私版原始檔走 LINE；設計區改成頁面清單避免表格文字過長，程式區依 HJ 架構擴寫。

## 下次安全起手式

1. 先讀 `memory/checkpoint.md` 的 2026-05-05 區塊。
2. 檢查 `git diff -- discussions/hj-quote-draft.md`。
3. 請 Wayne 先 review v12 金額、模組拆法與文字語氣。
