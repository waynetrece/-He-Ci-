...
const QUESTIONS = [
  {
    ...
    context:
      "目前畫面預設：① 企業客戶可正常申請與查看紀錄 ② 個人會員可申請但「每年限 X 件」（具體上限請貴司決定）。請貴司確認是否需要對個人會員設限。",
  ...
  },
  {
    ...
    context:
      "目前畫面預設五個狀態：① 申請中（剛送出）② 審核中（業務確認資格）③ 已寄出（出貨給物流）④ 已送達（收貨完成）⑤ 已關閉（拒絕／取消／逾期）。請貴司確認名稱與順序，以及是否與 ERP 同步。",
    pinnedAt: "篩選列『狀態』",
    clientRef: {
      source: "前台 / 公版商品系列 (2)",
      quote: "樣品申請：每個商品頁增加樣品按鈕",
