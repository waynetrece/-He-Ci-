...
const Q1 = {
  ...
  context:
    "目前先以這樣示意：① 訂單送出 → 網站存「待確認」狀態 → 即時 API 推給凌越 ERP → ERP 回 ack 後狀態變「已成立」② 若 ERP API 失敗 / 逾時，網站訂單先停在「待 ERP 同步」，背景重試（最多 3 次），仍失敗則通知 HJ 業務手動匯入。想請 HJ 確認：實際同步機制與失敗備援。",
  clientRef: {
    source: "後台 / 訂單管理 (3) + 後台 / 顧客管理 (5)",
    quote: "訂單即時匯入凌越 ERP；網站客人需與原 ERP 客戶編號相同",
    note: "需求表寫了「即時匯入凌越 ERP」，但失敗備援未寫。",
  },
};
...
const Q2 = {
  ...
  context:
    "目前先以這樣示意：① 月結訂單超過信用額度自動進「月結審核中」狀態 ② HJ 業務在後台審核，可「核准」或「拒絕並改其他付款方式」③ 核准後通知會員（Email / LINE）④ 拒絕則會員需重新到「結帳」改付款方式。想請 HJ 確認：審核權責、通知管道（與 LINE 整體 Q 對齊）、SLA。",
  clientRef: {
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-6 rounded-md border border-zinc-200 bg-white p-4 text-sm">
            ...
            <ul className="mt-2 space-y-1 text-xs text-zinc-600">
              ...
              <li>· LINE / 站內訊息：依會員設定（待 settings Q1 + Q5 確認）</li>
            </ul>
          </div>
        </div>
      </section>
