...
def build_hj():
    ...
    add_para(doc,
             "題目不全部做成選擇題；能用現況描述回答的地方，優先用開放問句，"
             "選項只作為提示方向。")
    ...
    questions = [
        (
            "一、樣品流程",
            "客戶同時加入樣品與公版商品時，HJ 目前希望怎麼處理？",
            None,
            "例如：是否需要分開結帳、同一頁送出後由系統拆成「樣品申請」與「一般訂單」、"
            "或可一起付款但後台分開出貨。若 HJ 已有既定作業方式，也可以直接說明現況。",
        ),
        (
            "二、私版詢價與 LINE",
        ...
        (
            "五、庫存同步速度",
            "HJ 對網站庫存更新速度的期待是什麼？是否需要接近即時、幾分鐘內更新"
            "（例如 5-10 分鐘），或只要客戶結帳前再次確認準確即可？",
            None,
            "不同期待會影響開發成本與技術風險，若 HJ 還沒有明確要求，"
            "也可以先描述目前可接受的延遲範圍。",
        ),
        (
            "六、庫存顯示與預留庫存",
        ...
        (
            "九、退貨退款",
            "退貨退款時，HJ 目前的處理流程是什麼？哪些動作由會計人工處理，"
            "哪些希望未來由系統連動？",
            None,
            "可一併說明信用卡退刷、匯款退款、發票作廢、凌越銷退或訂單狀態更新"
            "目前各由誰處理。",
        ),
        (
            "十、商品資料來源與訂單狀態",
